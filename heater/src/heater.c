...
s16 judge_threshold(s16 setTemp,s16 curTemp)
{
	...
	static s16 res = 0;
		...
		if( compare_timestamp(CurTick,UPDATE_TIME_MS))
		{
...
int heater_time_out_callback(void *p)
{
	HEATER *pheater = NULL;
	if(NULL == p)
	{
	...
	if(SysStat.pHtrS->sleep > HEATER_HALF_SLEEP)
	{
		...
		SysStat.pHtrS->setTemp = pheater->SetTemp;
		...
		else
		{
			...
			set_timer_time(SysStat.pHtrS->htrTmr,HEATER_SLEEP_MS);
			//reset_timer(SysStat.pHtrS->htrTmr);
			start_timer(SysStat.pHtrS->htrTmr);
		}
	}
	else if(HEATER_HALF_SLEEP == SysStat.pHtrS->sleep)
	{
		SysStat.pHtrS->sleep = HEATER_SLEEPED;
		SysStat.pHtrS->heating = COOLING;
		pheater->SetTemp = SysStat.pHtrS->setTemp;
...
void heat_alarm(void)
{
	...
	else if(SysStat.pHtrS->CurTemp > ALARM_TEMP)
	{
		...
		if(TempOverCnt >= 8000)//80000----->10600ms
		{
			SysStat.pHtrS->HtrErr |= HEATER_TEMP_OVER_ERR;
			TempOverCnt = 0;
			POWER_HEATER_OFF;//恢复正常后，PID会打开开关加热
		}
	}
	else
	{
	...
	{
		if( 0 == compare_timestamp(SysStat.pHtrS->CurOvertime,500) )
		{
			return ;//close heat(in EXTI3_IRQHandler )  but not do nothing when happen current over 
		}
		if(compare_timestamp(SysStat.pHtrS->alarmClock,1500) )
		{
	...
	if(SysStat.pHtrS->CurTemp <= HEATER_MIN_TEMP)
	{
		if(MinTempTime)
		{
			if(compare_timestamp(MinTempTime,HEATER_TEMP_LOW_TIMER) )
			{
...
void test_heater_alarm(void)
{
	...
	if(compare_timestamp(TestTimems,70000))
	{
		TestTimems = SysStat.Tmr2CntMs;
		SysStat.pHtrS->sleep = HEATER_WAKEN;
	}
	else if(compare_timestamp(TestTimems,60000))
	{
		SysStat.pHtrS->sleep = HEATER_SLEEPED;
	}
	else if(compare_timestamp(TestTimems,50000)
	{
		SysStat.pHtrS->HtrErr = HEATER_CUR_OVER_ERR;
	}
	else if(compare_timestamp(TestTimems,40000))
	{
		SysStat.pHtrS->HtrErr = HEATER_TEMP_LOW_ERR;
	}
	else if(compare_timestamp(TestTimems,30000))
	{
		SysStat.pHtrS->HtrErr = HEATER_TEMP_OVER_ERR;
	}
	else if(compare_timestamp(TestTimems,20000))
	{
		SysStat.pHtrS->HtrErr = HEATER_PWM_FREQ_ERR;
	}
	else if(compare_timestamp(TestTimems,10000))
	{
		SysStat.pHtrS->HtrErr = HEATER_TEMP_BOARD_ERR;
	}				
#endif
...
u16 calibrate_temp(void)
{
	u16 tempGap;
	
	if(SysStat.pHtrS->adActTemp > ph.htr.SetTemp)
	{
		tempGap = SysStat.pHtrS->adActTemp - ph.htr.SetTemp;
	}
	else
	{
		tempGap = ph.htr.SetTemp - SysStat.pHtrS->adActTemp;
	}
	return tempGap;
}
...
********************************************************************************/
u16 calc_display_temp(u16 surfaceTemp,u16 inTemp,u16 lastDispTemp)
{
	static u32 timerStamp = 0;
	//static u16 cnt         = 0;
	static u16 temp = 0;
	static u16 lastInTemp = 0;
	u16 curDispTemp       = 0;
	u16 tempIncrement      = 0;
	static u8 pwrOn = 1;
	static u8 heaterDiscon = 0;
	s8 rise = 0;

	if(inTemp > ALARM_TEMP)
	{
		SysStat.pHtrS->heaterFloat = 1;
		SysStat.pHtrS->heatFloatTime = SysStat.Tmr2CntMs;
		print_temp = 1;
		return 0xffff;
	}
	//解决烙铁接上去那一会检测温度不准确问题
	if( SysStat.pHtrS->heaterFloat && (compare_timestamp(SysStat.pHtrS->heatFloatTime,1000)) )
	{
		SysStat.pHtrS->heaterFloat = 0;
		if(inTemp > SysStat.pHtrS->adActTemp)
		{
			return SysStat.pHtrS->adActTemp;
		}
		else
		{
			return surfaceTemp;
	...
	temp = (temp?temp:inTemp);
	tempIncrement = abs(temp-inTemp);
	...
	if(surfaceTemp < inTemp)
	{//加热是开启的
		curDispTemp = lastDispTemp;
		if(curDispTemp < surfaceTemp)
		{
			curDispTemp += abs(inTemp - lastInTemp);
		...
		else
		{
			...
			{	
				...
				if(curDispTemp >= SysStat.pHtrS->adActTemp)
				{
					heaterDiscon = 1;//温度比较高认为是烙铁之前拔了，现在刚接上
					curDispTemp--;
				}
				else
				{
					heaterDiscon?curDispTemp--:curDispTemp++;
					if(curDispTemp <= ph.htr.SetTemp)
					{
						heaterDiscon = 0;
					}
				}
			...
			else 
			{
				if(compare_timestamp(timerStamp, 2000) )
				{
					if(tempIncrement == 1)
					{
						curDispTemp -= (inTemp-surfaceTemp)>>3;
					}						
					else
					{
						curDispTemp--;
		...
		if( abs(curDispTemp-surfaceTemp) <= 2 )
		{
			curDispTemp = surfaceTemp;
			heaterDiscon = 0;
		}
		
	}
	else if(surfaceTemp == inTemp)
	{
		timerStamp = SysStat.Tmr2CntMs;
		curDispTemp = lastDispTemp;
		//解决加热温度和设定温度相等时，关闭电源又开启动电源时，温度会显示烙铁内部温度(比关电源温度高)
		if(1==pwrOn)
		{
			if(SysStat.Tmr2CntMs < 10000)
			{
				//关电源前加热到了设定温度
				if(inTemp > (SysStat.pHtrS->adActTemp - 5) )
				{
					curDispTemp =  ph.htr.SetTemp - (calibrate_temp() >> 3);
				}
			}
		...
		if( abs(curDispTemp-inTemp) <= 2 )
		{
			curDispTemp = inTemp;
		}
		
		//解决上电前烙铁温度比较高，而开机第一次检测温度为33度问题。
		if( (SysStat.Tmr2CntMs <300) && ((curDispTemp < 50) && (inTemp > 100) ) )
		{
			curDispTemp = inTemp;
		}
	}
...
void heater_ctrl(HEATER*pheater,u16 *pAdValue,u8 update)
{
	...
	{
		pheater->SetTemp = HEATER_HALF_SLEEP_TEMP+22;
	}
	...
	{
		SysStat.pHtrS->temGap = calibrate_temp();
		if(SysStat.pHtrS->CurTemp > SysStat.pHtrS->temGap)
		{
			SysStat.pHtrS->CurTemp -= SysStat.pHtrS->temGap;
		}
		...
		if(SysStat.pHtrS->sleep >= HEATER_HALF_SLEEP)
		{
			SysStat.pHtrS->DispTemp = calc_display_temp(SysStat.pHtrS->CurTemp,//烙铁外部温度
			                            SysStat.pHtrS->AdTemp,//烙铁内部温度
	...
	//if(lastInTemp != SysStat.pHtrS->AdTemp)
	//if(print_temp && (lastAd != pAdValue[0]))
	if(0)
	{
		TEMP_DIS_DEBUG("%d %d %d %d\r\n",SysStat.Tmr2CntMs/1000,pAdValue[0],SysStat.pHtrS->AdTemp,SysStat.pHtrS->DispTemp);
		lastInTemp = SysStat.pHtrS->AdTemp;
		lastAd = pAdValue[0];
	}
...
#else		
		new_pid_run();
		//HeatTime = mohu_pid(SysStat.pHtrS->adActTemp,SysStat.pHtrS->CurTemp);
#endif
	}
...
void heater_stat_init(HEATER *pheater)
{
	...
	new_pid_init();	
#endif
	SysStat.pHtrS->htrTmr = creat_timer(HEATER_HALF_SLEEP_MS, 0, heater_time_out_callback,pheater);
