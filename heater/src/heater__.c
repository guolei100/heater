...
static u16 get_cur_dis_temp(u16 LastTemp,u16 curTemp)
{
	u16 temp = 0;
	if(LastTemp < curTemp)
	{
...
void heater_display(HEATER *heater)
{
	...
	if(balance)
	{
		if( SysStat.pHtrS->DispTemp > heater->SetTemp)
		{
			temp = (SysStat.pHtrS->DispTemp-heater->SetTemp);
			curTemp = heater->SetTemp + temp/2;
		}
		else
		{
			temp = (heater->SetTemp - SysStat.pHtrS->DispTemp);
			curTemp = heater->SetTemp - temp/2;
		}
	...
	if((SysStat.Tmr2CntMs - ticks) >= 500)
	{
		curTemp = get_cur_dis_temp(LastTemp,curTemp);
		ticks = SysStat.Tmr2CntMs;
		LastTemp = curTemp;
	}
